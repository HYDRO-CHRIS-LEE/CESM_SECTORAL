...
!$OMP END PARALLEL DO
  ...
  subroutine htape_timeconst(t, mode)
    ...
    use clm_instur      , only : irrig_method !chris
...
999    format(a,i0)
    ...
    if (mode == 'define' .and. tape(t)%ntimes == 1) then
       ...
       !chris
       !if (ldomain%isgrid2d) then
       !  call ncd_defvar(varname='irrigation_method' , xtype=ncd_int, &
       !      dim1name='lon', dim2name='lat', &
       !      long_name='index of irrigation method', ncid=nfid(t), &
       !      imissing_value=ispval, ifill_value=ispval)
       !else
       !  call ncd_defvar(varname='irrigation_method' , xtype=ncd_int, &
       !      dim1name=grlnd, &
       ...
       !call ncd_io(varname='irrigation_method' , data=irrig_method, dim1name=grlnd, ncid=nfid(t), flag='write')
       !call ncd_io(varname='irrigation_method', data=ldomain%irrigation_method, dim1name=grlnd, ncid=nfid(t), flag='write')
       !chris
    end if  ! (define/write mode
