...
!#10
   !-----------------------------------------------------------------------
   subroutine CalcSectorwaterSpecificYield(bounds, &
        num_soilc, filter_soilc, &
        soilhydrology_inst, soilstate_inst, sum_s_y)
     !
     ! !DESCRIPTION:
     ! Calculate and accumulate the sectoral specific yield from groundwater for each column.
     !
     ! This subroutine calculates the sectoral specific yield from groundwater for each soil column
     ! within specified bounds. It is called when groundwater sectoral water demand is considered.
     ! The specific yield represents the portion of soil water that can be released as groundwater
     ! storage when the water table rises above a particular soil layer.
     !
     ! This routine is called when use_sectorwater_irrigation = .true.
     !
     ! It is not compatible with use_aquifer_layer
     !
     ! !ARGUMENTS:
     ...
     type(soilhydrology_type) , intent(in)    :: soilhydrology_inst
     type(soilstate_type)     , intent(in)    :: soilstate_inst
     real(r8)                 , intent(out)   :: sum_s_y(:)

     !
     ! !LOCAL VARIABLES:
     character(len=32) :: subname = 'CalcSectorwaterSpecificYield'  ! subroutine name
     integer  :: c,j,fc                                  ! indices
     real(r8) :: dtime                                   ! land model time step (sec)
     integer  :: jwt(bounds%begc:bounds%endc)            ! index of the soil layer right above the water table (-)
     real(r8) :: s_y
     ...
     !-----------------------------------------------------------------------
     ! Initialize specific_yield_sum to zero
     sum_s_y(:) = 0.0_r8
   ...
   end subroutine CalcSectorwaterSpecificYield
!#11
   !-----------------------------------------------------------------------
   subroutine WithdrawGroundwaterSectorwater(bounds, &
     ...
     SHR_ASSERT_ALL_FL((ubound(qflx_gw_uncon_sectorwater_lyr) == [bounds%endc, nlevsoi]), sourcefile, __LINE__)
     ...
     do j = 1, nlevsoi
        do fc = 1, num_soilc
           c = filter_soilc(fc)
           qflx_gw_uncon_sectorwater_lyr(c,j) = 0._r8
        end do
     end do
     ...
     do j = 1, nlevsoi
        do fc = 1, num_soilc
           c = filter_soilc(fc)
           h2osoi_liq(c,j) = h2osoi_liq(c,j) - qflx_gw_uncon_sectorwater_lyr(c,j) * dtime
        end do
     end do
     ...
     do fc = 1, num_soilc
        c = filter_soilc(fc)
        wa(c) = wa(c) - qflx_gw_con_sectorwater(c) * dtime
     end do

     end associate

   end subroutine WithdrawGroundwaterSectorwater
