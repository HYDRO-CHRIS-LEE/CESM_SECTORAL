...
#include "shr_assert.h"
  ...
  use pftconMod        , only : nirrig_rice !chris added
  !
  implicit none
  private
  ...
  ! Flood is applied directly to soil surface as well, but with a different amount !chris added
...
contains
  ...
  subroutine IrrigationInitHistory(this, bounds)
    ...
    !chris5
    use clm_varpar     , only : cft_size, cft_lb, cft_ub !chris5 !chris6
    use shr_infnan_mod , only : nan => shr_infnan_nan, assignment(=) !chris5
    !
    ! !ARGUMENTS:
    class(irrigation_type) , intent(inout) :: this
    type(bounds_type)      , intent(in)    :: bounds
    ...
    ! !LOCAL VARIABLES:
    integer :: begp, endp
    real(r8), target, allocatable :: irrig_method_patch_real(:)
    real(r8), pointer :: ptr_irrig_method_patch_real(:)

    real(r8), target, allocatable :: irrig_method_patch_real1(:)
    real(r8), pointer :: ptr_irrig_method_patch_real1(:)

    real(r8), target, allocatable :: irrig_method_patch_real2(:,:) !chris5
    real(r8), pointer :: ptr_irrig_method_patch_real2(:,:)    

    real(r8), target, allocatable :: irrig_method_patch_real22(:,:) !chris7
    real(r8), pointer :: ptr_irrig_method_patch_real22(:,:)

    real(r8), target, allocatable :: irrig_method_patch_real3(:,:) !chris8
    real(r8), pointer :: ptr_irrig_method_patch_real3(:,:)

    real(r8), target, allocatable :: irrig_method_patch_real33(:,:) !chris8
    real(r8), pointer :: ptr_irrig_method_patch_real33(:,:)
    ...
    !chris4
    !allocate(irrig_method_patch_real(begp:endp))
    !irrig_method_patch_real = real(this%irrig_method_patch(begp:endp), kind=r8)
    !ptr_irrig_method_patch_real => irrig_method_patch_real
    !ptr_irrig_method_patch_real(:) = nan
    !call hist_addfld1d (fname='IRRIG_METHOD 1d', units='unitless', &
    !     avgflag='A', long_name='irrigation method 1d patch', &
    !     ptr_patch=ptr_irrig_method_patch_real)


    !allocate(irrig_method_patch_real1(begp:endp))
    !irrig_method_patch_real1 = real(this%irrig_method_patch(begp:endp), kind=r8)
    !ptr_irrig_method_patch_real1 => irrig_method_patch_real1
    !ptr_irrig_method_patch_real1(:) = nan
    !call hist_addfld1d (fname='IRRIG_METHOD 1d p2c2l', units='unitless', &
    !     avgflag='A', long_name='irrigation method 1d patch p2c2l', &
    !     ptr_patch=ptr_irrig_method_patch_real1, &
  ...
  subroutine CalcIrrigationNeeded(this, bounds, num_exposedvegp, filter_exposedvegp, &
    ...
    ! target soil moisture for this layer [kg/m2] (target for drip and spri; target_satu for flood and paddy) !chris added
    real(r8) :: h2osoi_liq_target
    real(r8) :: h2osoi_liq_target_satu !chris added
    ...
    real(r8) :: h2osoi_liq_target_satu_tot(bounds%begc:bounds%endc)	!chris added
    ...
    real(r8) :: deficit_satu(bounds%begc:bounds%endc)	!chris added										  
    real(r8) :: deficit_pool(bounds%begc:bounds%endc)	!chris added
    ...
    do fc = 1, check_for_irrig_col_filter%num
       ...
       h2osoi_liq_target_satu_tot(c) = 0._r8	!chris added
          ...
          if (.not. reached_max_depth(c)) then
             if (col%z(c,j) > this%params%irrig_depth) then
                ...
                h2osoi_liq_target_tot(c) = h2osoi_liq_target_tot(c) + &
                     h2osoi_liq_target
                ! Here we calculate the saturated target value !chris added
    ...
    deficit_satu(bounds%begc:bounds%endc) = 0._r8 !chris added
    deficit_pool(bounds%begc:bounds%endc) = 0._r8 !chris added
    do fc = 1, check_for_irrig_col_filter%num
       c = check_for_irrig_col_filter%indices(fc)
       ...
       if (h2osoi_liq_tot(c) < h2osoi_liq_at_threshold) then
          ...
          deficit_satu(c) = h2osoi_liq_target_satu_tot(c) - h2osoi_liq_tot(c) !chris added
          deficit_pool(c) = h2osoi_liq_target_satu_tot(c) - h2osoi_liq_tot(c) !chris added
       ...
       ! For paddy, irrigation is acctivated once soil water is not saturated !chris added
         ...
         deficit_pool(c) = h2osoi_liq_target_satu_tot(c) - h2osoi_liq_tot(c) 
          ...
          deficit(c) = 0._r8
          deficit_satu(c) = 0._r8 !chris added
          deficit_pool(c) = 0._r8 !chris added
       end if
    end do
    ...
    if (limit_irrigation) then
       ...
       deficit_satu_volr_limited(bounds%begc:bounds%endc) = deficit_satu(bounds%begc:bounds%endc) !chris added
       deficit_pool_volr_limited(bounds%begc:bounds%endc) = deficit_pool(bounds%begc:bounds%endc) !chris added
    end if
    ...
    do fp = 1, num_exposedvegp
       ...
       if (check_for_irrig_patch(p)) then
         ...
         !  this%sfc_irrig_rate_patch(p) = deficit_volr_limited(c) / & !chris removed
         !       (this%dtime*this%irrig_nsteps_per_day)
         !  this%irrig_rate_demand_patch(p) = deficit(c) / &
         !       (this%dtime*this%irrig_nsteps_per_day)

         ! This is for irrigated rice column !chris added
